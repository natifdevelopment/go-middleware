		...
		if sso.ValidateGatewaySecret(c.Request) {
			...
			if authHeader := c.GetHeader("Authorization"); authHeader != "" {
				if claims, err := sso.ParseJWT(authHeader); err == nil {
					...
					// If the JWT did not contain a user claim (e.g. tokens that
					// only carry sub/exp/iat/type), fall back to X-User-Id so
					// downstream handlers still have a user identity.
					if claims.User == nil {
						setUserFromHeader(c)
					}
					c.Next()
...
// setUserFromHeader sets a minimal user context from the X-User-Id header.
func setUserFromHeader(c *gin.Context) {
...
// setClaimsContext sets user/page/activity/pageActivity in the Gin context
// from JWT claims, but only when the claim is non-nil. This prevents
// overwriting values set by earlier middleware (e.g. PageActivityMiddleware)
// when the JWT does not contain full context claims (which is the case for
// tokens generated by the auth service that only carry sub/exp/iat/type).
func setClaimsContext(c *gin.Context, claims *sso.JWTClaims) {
	if claims.User != nil {
	...
	if claims.Activity != nil {
		c.Set("activity", claims.Activity)
	}
	if claims.PageActivity != nil {
